...
try {
    ...
    for ($r = 2; $r -le $rowCount; $r++) {
        ...
        if ($subStr -eq $null) { $subStr = "[NULL]" }
        
        $code = ""
        if ($subStr -match '^(\d{3})') { $code = $matches[1] }
        
        if ($code -eq "" -or -not $mapping.ContainsKey($code)) {
            $otherSubs[$subStr]++
        }
    }
    
    Write-Host "Unmapped Subsidiary Names:"
    $otherSubs.Keys | ForEach-Object {
        Write-Host "$_ : $($otherSubs[$_])"
    }
